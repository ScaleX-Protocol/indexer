...
const initRedisClient = async (): Promise<RedisClientType | null> => {
  try {
    if (!redisClient) {
...
async function setWebSocketEnableBlockNumber(): Promise<void> {
  try {
...
export { setWebSocketEnableBlockNumber, getCurrentBlockNumber, setCachedData };